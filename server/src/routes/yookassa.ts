...
router.post('/payments/yookassa/create', async (req: Request, res: Response) => {
  ...
  try {
    const planRow = await query<{ id: string; price_rub: number; duration_days: number }>(
      `SELECT id, price_rub, duration_days FROM plans WHERE id = $1 AND is_active = true`,
      [planId.trim()]
    )
    ...
    const description = `GameNight Host Premium ${plan.duration_days} days`
    const body = {
      amount: { value: amount, currency: 'RUB' },
      confirmation: { type: 'redirect' as const, return_url: returnUrl },
      capture: true,
      description,
      metadata: { telegramId: String(telegramId), planId: plan.id },
    }
    ...
    await query(
      `INSERT INTO payments (
         telegram_id, provider, plan_id, currency, amount,
         provider_payment_charge_id, telegram_payment_charge_id,
         invoice_payload, status
       ) VALUES ($1, 'yookassa', $2, 'RUB', $3, $4, NULL, $5, 'pending')`,
      [
        telegramId,
        plan.id,
        plan.price_rub,
    ...
    res.status(200).json({ ok: true, confirmationUrl })
  } catch (e) {
...
router.post('/payments/yookassa/webhook', async (req: Request, res: Response) => {
  ...
  const body = req.body as { type?: string; event?: string; object?: { id?: string; status?: string; metadata?: { telegramId?: string; planId?: string } } }
  const eventType = body.event ?? body.type
  if (eventType !== 'payment.succeeded' || !body.object) {
  ...
  const telegramId = metadata.telegramId ? parseInt(metadata.telegramId, 10) : null
  const planId = metadata.planId

  if (!paymentId || !telegramId || !planId) {
    console.error('[YooKassa] webhook: missing paymentId, telegramId or planId')
    res.status(200).send('OK')
    return
  }
  ...
  try {
    ...
    const plan = planRow.rows[0]
    if (!plan) {
      console.error('[YooKassa] webhook: plan not found', planId)
      res.status(200).send('OK')
      return
    }
